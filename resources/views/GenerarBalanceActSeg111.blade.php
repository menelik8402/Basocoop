                                ...
                                @foreach($progs as $key => $prog)
                                    @foreach($prog->Metas as $kil => $met)
                                        @foreach($met->GetSeguimientos as $seg => $seguimiento)
                                          ...
                                          <table class="table  table-bordered table-sm  table-hover tabla">
                                        <div class="alert alert-info">  Información sobre el programa "{{$prog->nomb_prog}}" , actividad "{{$met->desc_unid_fisicas}}" seguimiento de la Actividad  "{{$met->desc_unid_fisicas}}"</div>
                                        ...
                                        ?>
                                            ...
                                            <tr>
                                                ...
                                                @if((round($seguimiento->num_beneficiarios_real/$seguimiento->num_benef_planif,2)*100)> 100 )
                                                    <td>{{round($seguimiento->num_beneficiarios_real/$seguimiento->num_benef_planif*100 * $seguimiento->num_benef_planif*100/$total_NB/100,2)}} <small>ASA</small> </td>
                                                    <td>0</td>
